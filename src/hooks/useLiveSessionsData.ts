...
export type LiveSessionsRow = SheetRecord;

const BACKGROUND_REFRESH_MS = 60 * 1000;

export function useLiveSessionsData() {
    const { programId, config } = useProgramConfig();

    const [rows, setRows] = useState<LiveSessionsRow[] | null>(null);
        ...
        async function load(options?: { background?: boolean }) {
            ...
            try {
                const data = await fetchSheetTab({
                    ...
                    sheetName: SHEET_TABS.liveSessions,
                });
