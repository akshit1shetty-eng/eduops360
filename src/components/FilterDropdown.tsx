...
export default function FilterDropdown({
  ...
  const effectiveCount = count ?? safeSelected.length;
    ...
    <div className="relative group/filter" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <i className={`${iconClass} text-xs ${effectiveCount > 0 ? 'text-indigo-600' : 'text-gray-400'}`} />
          <span className="text-sm truncate">
            {effectiveCount > 0 ? `${effectiveCount} Selected` : placeholder}
          </span>
        </div>
        <i
          className={`fas fa-chevron-down text-[10px] transition-transform duration-300 ${
            isOpen ? 'rotate-180' : ''
          } ${effectiveCount > 0 ? 'text-indigo-400' : 'text-gray-300'}`}
        />
      </button>

      {isOpen && (
        <div className="absolute z-50 w-full min-w-[200px] mt-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-72">
          {children ? (
            <div className="overflow-y-auto p-1 flex-1">{children}</div>
          ) : (
            <>
              {/* Search Input Box */}
