...
import { useProgramBudgetData } from '../../hooks/useBudgetData';
import { PROGRAMS } from '../../lib/config';

export function getActualsCardConfig(key: string) {
  const lower = key.toLowerCase();

  if (lower.includes('doubt') || lower.includes('resolution') || lower.includes('call')) {
    return { label: lower.includes('session') ? 'Doubt Resolution Session' : 'Doubt Resolution', color: '#06b6d4', icon: 'fa-comments' };
  }
  if (lower.includes('live') || lower.includes('session')) {
    return { label: 'Live Session', color: '#6366f1', icon: 'fa-video' };
  }
  if (lower.includes('assignment') || lower.includes('grading')) {
    return { label: 'Assignment Grading', color: '#10b981', icon: 'fa-tasks' };
  }
  if (lower.includes('thesis') && lower.includes('workshop')) {
    return { label: 'Thesis Workshop', color: '#f59e0b', icon: 'fa-chalkboard-teacher' };
  }
  if (lower.includes('thesis') && lower.includes('defence')) {
    return { label: 'Thesis Defence', color: '#d97706', icon: 'fa-shield-alt' };
  }
  if (lower.includes('thesis') && lower.includes('masterclass')) {
    return { label: 'Thesis Masterclass', color: '#b45309', icon: 'fa-university' };
  }
  if (lower.includes('thesis') && lower.includes('co-chair')) {
    return { label: 'Thesis Co-Chair', color: '#78350f', icon: 'fa-user-friends' };
  }
  if (lower.includes('thesis') && lower.includes('grading')) {
    return { label: 'Thesis Grading', color: '#10b981', icon: 'fa-marker' };
  }
  if (lower.includes('thesis') && (lower.includes('supervision') || lower.includes('supervisoon'))) {
    return { label: 'Thesis Supervision', color: '#eab308', icon: 'fa-book-open' };
  }
  if (lower.includes('ta ') || lower.includes(' ta') || lower === 'ta actuals') {
    return { label: 'TA Actuals', color: '#8b5cf6', icon: 'fa-user-tie' };
  }
  if (lower.includes('df') || lower.includes('discussion')) {
    return { label: 'DF Cost', color: '#3b82f6', icon: 'fa-graduation-cap' };
  }
  if (lower.includes('event') || lower.includes('immersion')) {
    return { label: 'Events / Immersion', color: '#f97316', icon: 'fa-star' };
  }
  if (lower.includes('other')) {
    return { label: 'Others', color: '#ec4899', icon: 'fa-ellipsis-h' };
  }
  ...
  return { label: cleanLabel || key, color: '#64748b', icon: 'fa-dollar-sign' };
}
...
function CohortBreakdownTable({
  ...
  const tillDateFormatted = useMemo(() => {
    if (!effectiveFY) return '';
    if (monthFilter) return monthFilter;
    const today = new Date();
    const lastDay = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
    const now = today.getDate() === lastDay ? today : new Date(today.getFullYear(), today.getMonth(), 0);
    const actualCurrentFYStartYear = now.getMonth() >= 3 ? now.getFullYear() : now.getFullYear() - 1;
    const isActualFY = effectiveFY === actualCurrentFYStartYear;
    const d = isActualFY ? now : new Date(effectiveFY + 1, 2, 31);
    const mmm = d.toLocaleDateString('en-US', { month: 'short' });
    const yy = d.toLocaleDateString('en-US', { year: '2-digit' });
    return `${mmm}-${yy}`;
  }, [effectiveFY, monthFilter]);

  // Filter cohorts active in selected FY
  const activeCohorts = useMemo(() => {
    return cohorts.filter(c => c.currentFYBudgeted !== 0 || c.currentFYUtilized !== 0);
  }, [cohorts]);

  // Apply search query
  const filteredCohorts = useMemo(() => {
    return activeCohorts.filter(c =>
      c.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [activeCohorts, searchQuery]);

  const currFYStart = useMemo(() => {
    return effectiveFY ? new Date(effectiveFY, 3, 1) : null;
  }, [effectiveFY]);

  const currFYEnd = useMemo(() => {
    return effectiveFY ? new Date(effectiveFY + 1, 2, 31) : null;
  }, [effectiveFY]);
  ...
  const cutoffDate = useMemo(() => {
    ...
    const actualCurrentFYStartYear = now.getMonth() >= 3 ? now.getFullYear() : now.getFullYear() - 1;
    const isActualFY = effectiveFY === actualCurrentFYStartYear;
    return isActualFY ? now : new Date(effectiveFY + 1, 2, 31);
  }, [effectiveFY, monthFilter]);

  const processedCohorts = useMemo(() => {
    if (!effectiveFY || !currFYStart || !currFYEnd || !cutoffDate) return [];

    return filteredCohorts.map(row => {
      const totalBudgeted = row.months
        .filter((m: any) => m.month >= currFYStart && m.month <= currFYEnd)
        .reduce((sum: number, m: any) => sum + (m.amount || 0), 0);

      const budgetedTill = row.months
        .filter((m: any) => m.month >= currFYStart && m.month <= currFYEnd && m.month <= cutoffDate)
        .reduce((sum: number, m: any) => sum + (m.amount || 0), 0);

      const actualsTill = row.months
        .filter((m: any) => m.month >= currFYStart && m.month <= currFYEnd && m.month <= cutoffDate)
        .reduce((sum: number, m: any) => sum + (m.actuals || 0), 0);

      const deviation = budgetedTill - actualsTill;
      const deviationPct = budgetedTill !== 0 ? (deviation / budgetedTill) * 100 : actualsTill !== 0 ? -100 : 0;
  ...
  }, [filteredCohorts, currFYStart, currFYEnd, cutoffDate, effectiveFY]);

  const tableHeader = (
    <tr className="border-b border-slate-200">
      <th className="py-4 px-4 text-[9px] font-black text-slate-500 uppercase tracking-[0.12em] bg-slate-50 w-[20%]">Cohort</th>
      <th className="py-4 px-4 text-[9px] font-black text-slate-500 uppercase tracking-[0.12em] text-right bg-slate-50 w-[14%]">Total Budgeted</th>
      <th className="py-4 px-4 text-[9px] font-black text-indigo-600 uppercase tracking-[0.12em] text-right bg-slate-50 w-[18%]">Budgeted till ({tillDateFormatted})</th>
      <th className="py-4 px-4 text-[9px] font-black text-orange-600 uppercase tracking-[0.12em] text-right bg-slate-50 w-[18%]">Actuals ({tillDateFormatted})</th>
      <th className="py-4 px-4 text-[9px] font-black text-slate-500 uppercase tracking-[0.12em] text-right bg-slate-50 w-[16%]">Deviation ($)</th>
      <th className="py-4 px-4 text-[9px] font-black text-slate-500 uppercase tracking-[0.12em] text-right bg-slate-50 w-[14%]">Deviation %</th>
    </tr>
  );
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg">
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 uppercase tracking-wider">Cohort Breakdown</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                {effectiveFY
                  ? `FY (Apr-${String(effectiveFY).slice(-2)} to Mar-${String(effectiveFY + 1).slice(-2)}) · Click a cohort to view detailed breakdown`
                  : 'Select an FY from the dropdown above'}
              </p>
            </div>
          </div>
          <div className="relative">
            <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <i className="fas fa-search text-slate-400 text-xs" />
            </span>
            <input
              type="text"
              value={searchQuery}
              ...
              className="pl-8 pr-4 py-1.5 w-full sm:w-60 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 transition-all duration-300 shadow-sm"
            />
          </div>
        </div>
        ...
        <div className="overflow-y-auto max-h-[600px] custom-scrollbar">
          <table className="w-full text-left border-collapse table-fixed">
            <thead className="sticky top-0 z-20">
              {tableHeader}
              {!loading && effectiveFY && processedCohorts.length > 0 && (
                (() => {
                  const totalBudgeted = processedCohorts.reduce((s, r) => s + r.totalBudgeted, 0);
                  const totalBudgetedTill = processedCohorts.reduce((s, r) => s + r.budgetedTill, 0);
                  const totalActuals = processedCohorts.reduce((s, r) => s + r.actualsTill, 0);
                  const totalDev = totalBudgetedTill - totalActuals;
                  const totalDevPct = totalBudgetedTill !== 0 ? (totalDev / totalBudgetedTill) * 100 : totalActuals !== 0 ? -100 : 0;
                  const isOver = totalDev < 0;
                  const isUnder = totalDev > 0;

                  return (
                    <tr className="border-b-2 border-slate-200 bg-slate-50">
                      <td className="py-4 px-4 bg-slate-50">
                        <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Total</span>
                      </td>
                      <td className="py-4 px-4 text-right bg-slate-50">
                        <div className="text-[13px] font-black text-slate-900">{formatCompact(totalBudgeted)}</div>
                        <div className="text-[10px] font-bold text-slate-400 mt-0.5">{formatCurrency(totalBudgeted)}</div>
                      </td>
                      <td className="py-4 px-4 text-right bg-slate-50">
                        <div className="text-[13px] font-black text-indigo-700">{formatCompact(totalBudgetedTill)}</div>
                        <div className="text-[10px] font-bold text-indigo-500/70 mt-0.5">{formatCurrency(totalBudgetedTill)}</div>
                      </td>
                      <td className="py-4 px-4 text-right bg-slate-50">
                        <div className="text-[13px] font-black text-orange-600">{formatCompact(totalActuals)}</div>
                        <div className="text-[10px] font-bold text-orange-500/70 mt-0.5">{formatCurrency(totalActuals)}</div>
                      </td>
                      <td className="py-4 px-4 text-right bg-slate-50">
                        <div className={`text-[13px] font-black tracking-tight ${isOver ? 'text-red-500' : isUnder ? 'text-emerald-600' : 'text-slate-400'}`}>
                          {totalDev >= 0 ? '+' : ''}{formatCompact(totalDev)}
                        </div>
                        <div className={`text-[10px] font-bold mt-0.5 ${isOver ? 'text-red-400/70' : isUnder ? 'text-emerald-500/70' : 'text-slate-400'}`}>
                      ...
                      <td className="py-4 px-4 text-right bg-slate-50">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-black ${
                          isOver ? 'bg-red-50 text-red-600' : isUnder ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'
                        }`}>
                          {isOver ? <i className="fas fa-arrow-up text-[8px]" /> : isUnder ? <i className="fas fa-arrow-down text-[8px]" /> : null}
                          {Math.abs(totalDevPct).toFixed(1)}%
                        </span>
                      </td>
                    </tr>
                  );
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-16 text-center">
                    <div className="inline-flex items-center gap-3 text-slate-400">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider">Loading Cohort Data...</span>
                    </div>
                  </td>
                </tr>
              ) : !effectiveFY ? (
                <tr>
                  <td colSpan={6} className="py-16 text-center text-slate-400 text-xs font-bold uppercase tracking-widest italic bg-slate-50/20">
                    No FY selected
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={6} className="py-16 text-center text-slate-400 text-xs font-bold uppercase tracking-widest italic bg-slate-50/20">
                    {searchQuery ? 'No cohorts match your search query' : 'No cohort data available for this FY'}
                  </td>
                </tr>
              ) : (
                <>
                  {processedCohorts.map(row => {
                    const { totalBudgeted, budgetedTill, actualsTill, deviation, deviationPct, isOver, isUnder } = row;

                    return (
                      <tr
                        key={row.name}
                        onClick={() => navigate(`/lex/budget/${programId}/${row.name}?fy=${effectiveFY}`)}
                        className="transition-all border-l-4 border-transparent hover:bg-indigo-50/40 hover:border-indigo-300 cursor-pointer group"
                      >
                        {/* Cohort Name */}
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-xl flex items-center justify-center text-white text-[9px] bg-indigo-400 group-hover:bg-indigo-600 shadow-sm transition-colors">
                              <i className="fas fa-users" />
                            </div>
                            <div>
                              <span className="text-[13px] font-black uppercase tracking-wide text-slate-900 group-hover:text-indigo-900 transition-colors">
                                {row.name}
                              </span>
                            </div>
                          </div>
                        ...
                        <td className="py-4 px-4 text-right">
                          <div className="text-[13px] font-black text-slate-900 tracking-tight">{formatCompact(totalBudgeted)}</div>
                          <div className="text-[10px] font-bold text-slate-400 mt-0.5">{formatCurrency(totalBudgeted)}</div>
                        </td>
                        {/* Budgeted till */}
                        <td className="py-4 px-4 text-right">
                          <div className="text-[13px] font-black text-indigo-700 tracking-tight">{formatCompact(budgetedTill)}</div>
                          <div className="text-[10px] font-bold text-indigo-500/70 mt-0.5">{formatCurrency(budgetedTill)}</div>
                        </td>
                        {/* Actuals */}
                        <td className="py-4 px-4 text-right">
                          <div className="text-[13px] font-black text-orange-600 tracking-tight">{formatCompact(actualsTill)}</div>
                          <div className="text-[10px] font-bold text-orange-500/70 mt-0.5">{formatCurrency(actualsTill)}</div>
                        </td>
                        {/* Deviation $ */}
                        <td className="py-4 px-4 text-right">
                          <div className={`text-[13px] font-black tracking-tight ${isOver ? 'text-red-500' : isUnder ? 'text-emerald-600' : 'text-slate-400'}`}>
                            {deviation >= 0 ? '+' : ''}{formatCompact(deviation)}
                          </div>
                          <div className={`text-[10px] font-bold mt-0.5 ${isOver ? 'text-red-400/70' : isUnder ? 'text-emerald-500/70' : 'text-slate-400'}`}>
...
export default function LexProgramBudgetDetail() {
  ...
  const selectedYear = actualsSplitFilter ? Number(actualsSplitFilter) : fyStartYear;
  const { programStats, programActuals, programBudget, loading, error, cohorts = [] } = useProgramBudgetData(programId || '', selectedYear);
  ...
  // Available months for the selected FY, sorted Apr → Mar
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-500">
        ...
        <p className="text-lg font-bold">Error loading budget data</p>
        <p className="text-sm">{error}</p>
        <button
          onClick={() => navigate('/lex/budget')}
    ...
    <div className="max-w-[90rem] mx-auto py-2">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">
          ...
          <i className="fas fa-chevron-right text-[8px] opacity-50" />
          <span className="text-indigo-600">{programName}</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate('/lex/budget')}
              className="w-10 h-10 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:border-indigo-100 hover:bg-indigo-50 transition-all duration-300"
            >
              <i className="fas fa-arrow-left text-xs" />
            </button>
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-1">
                {programName} <span className="text-indigo-700">Budget Detail</span>
              </h1>
              <p className="text-slate-600 text-base font-medium opacity-80">Cohort-level financial breakdown and trends.</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          ...
          <div className="relative z-10 flex flex-col justify-between h-full">
            ...
            <div>
              <div className="text-2xl font-black text-slate-900 tracking-tighter mb-0">
                {loading ? <StatSkeleton /> : formatCompact(programStats?.currentFYBudgeted || 0)}
              </div>
              {!loading && (
                <div className="text-[11px] font-bold text-slate-400 tracking-wide mb-1">
                  ({formatCurrency(programStats?.currentFYBudgeted || 0)})
                </div>
              )}
        ...
        <div className="bg-white rounded-[2rem] p-5 border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-lg relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col justify-between h-full">
            ...
            <div>
              <div className="text-2xl font-black text-slate-900 tracking-tighter mb-0">
                {loading ? <StatSkeleton /> : formatCompact(programStats?.currentFYUtilized || 0)}
              </div>
              {!loading && (
                <div className="text-[11px] font-bold text-slate-400 tracking-wide mb-1">
                  ({formatCurrency(programStats?.currentFYUtilized || 0)})
                </div>
              )}
        ...
        <div className="bg-white rounded-[2rem] p-5 border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-lg relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col justify-between h-full">
            ...
            <div>
              <div className="text-2xl font-black text-slate-900 tracking-tighter mb-0">
                {loading ? <StatSkeleton /> : formatCompact(programStats?.previousFYBudgeted || 0)}
              </div>
              {!loading && (
                <div className="text-[11px] font-bold text-slate-400 tracking-wide mb-1">
                  ({formatCurrency(programStats?.previousFYBudgeted || 0)})
                </div>
              )}
        ...
        <div className="bg-white rounded-[2rem] p-5 border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-lg relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col justify-between h-full">
            ...
            <div>
              <div className="text-2xl font-black text-slate-900 tracking-tighter mb-0">
                {loading ? <StatSkeleton /> : formatCompact(programStats?.previousFYUtilized || 0)}
              </div>
              {!loading && (
                <div className="text-[11px] font-bold text-slate-400 tracking-wide mb-1">
                  ({formatCurrency(programStats?.previousFYUtilized || 0)})
                </div>
              )}
              <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest opacity-80 mt-1">{prevFYStartFormatted} to {prevFYEndFormatted}</div>
            </div>
              ...
              <div className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="colorBudgetProg" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#4f46e5" stopOpacity={0.1} />
                        <stop offset="95%" stopColor="#4f46e5" stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="colorActualsProg" x1="0" y1="0" x2="0" y2="1">
